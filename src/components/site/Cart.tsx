  ...
  async function finalize() {
    ...
    try {
      ...
      const msg = [
        ...
        ...items.map(
          (i) =>
            `• ${i.qty}x ${i.name}${i.size ? ` · Tam ${i.size}` : ""}${i.color ? ` · ${i.color}` : ""} — ${brl(i.price * i.qty)}`,
        ),
        ``,
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-center p-8 gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Adicione peças da coleção.</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-muted/20">
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <Input value={form.whatsapp} onChange={(e) => setForm({ ...form, whatsapp: e.target.value })} placeholder="(DDD) 9 9999-9999" />
          </div>
          <div>
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => setCheckout(false)} className="flex-1">
            Voltar
          </Button>
        ...
        {items.map((i) => {
          ...
          return (
            <div key={k} className="flex gap-3 p-4">
              {i.image && (
                <img src={i.image} alt={i.name} className="size-20 object-cover rounded border" />
              )}
              <div className="flex-1 min-w-0">
                <p className="font-medium leading-tight truncate">{i.name}</p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  {[i.size && `Tam ${i.size}`, i.color].filter(Boolean).join(" · ")}
                </p>
